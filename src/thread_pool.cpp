...
ThreadLoop::ThreadLoop(MPMCQueueAtomic<TPCallable>& taskQueueIn,
                       std::atomic_uint64_t& completedTaskCountIn,
                       uint32_t threadNumberIn)
    : taskQueue(taskQueueIn)
    , completedTaskCount(completedTaskCountIn)
...
void ThreadPool::RestartThreadsImpl(uint32_t threadCount)
{
    Wait();
    taskQueue.Terminate();
    threads.clear();
    ...
    {
        threads.emplace_back(ThreadLoop(taskQueue, completedTaskCount, i));
    }
